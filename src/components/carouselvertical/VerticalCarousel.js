...
export default class VerticalCarousel extends Component {
  constructor(props) {
    ...
    let {rotateX, translateZ} = this.computeTransformations(sides);
  ...
  getCardStyle(position) {
    return {
      transform: `rotateX(${position * this.state.rotateX}deg) translateZ(${this.state.translateZ}px)`,
  ...
  render() {
    let carouselRotation = -this.state.rotationIndex * this.state.rotateX;
    let carouselStyle = {
      transform: `translate(-50%, -50%) translateZ(${this.computeZoomOut()}px) rotateX(${carouselRotation}deg)`
    }
